...
class ParallelBMF:
    ...
    def fit(self, *data, initial_temp=10, decay_rate=0.88, period=10,
        ...
        # (latent_prior.temp) anneals on its own schedule when `prior_temp` is given
        # (only a structured prior reads it), and `mask` switches on imputation --
        # each chain fills the masked entries from its own reconstruction inside
        # grad_step.  One annealing schedule is shared across all C chains.
        if max_iter is None:
