"""
new_bae_models.py  --  the models
=================================

A refactor of `bae_models.py` that pulls the structure shared by every BMF model
...
instead of copy-pasted into ~10 classes.  Three modules:
  ...
  new_bae_weights.py  the weight side (one slot: `operator`)
...
class BMF:
    ...
    def fit(self, *data, initial_temp=10, decay_rate=0.88, period=10, min_temp=1,
            prior_schedule=None, prior_delay=0, prior_refit=False,
            refine_iters=0, refine_sweeps=2, refine_lr_scale=0.5,
            max_iter=None, verbose=True, mask=None, probes=None, **opt_args):
...
class KernelBMF(BMF):
    ...
    E-step's likelihood field (new_bae_search.make_kernel_search) and the scale
