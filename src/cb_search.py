...
def gecbm(Xhat: np.ndarray, 	# same shape as X 
		  X: np.ndarray,		# inputs
		  M: np.ndarray,		# input weights
          Yhat: np.ndarray,   	# same shape as Y
          Y: np.ndarray, 		# outputs
          W: np.ndarray,		# output weights
          S: np.ndarray, 		# initial conditions
          temp: float,
          lnx: bae_util.gaussian,
          lny: bae_util.gaussian,
    ...
    # for i in range(n):
        ...
        # for j in range(m):
            ...
            dot = 0    
            for k in range(dx):
                dot += gamma*lnx(Xhat[i,k] + (1-Sij)*M[j,k])
                dot -= gamma*lnx(Xhat[i,k] - Sij*M[j,k])
            for k in range(dy):
                dot += lny(Yhat[i,k] + (1-Sij)*W[j,k])
                dot -= lny(Yhat[i,k] - Sij*W[j,k])
            ...
            curr = (gamma*XM[i,j] + YW[i,j] - dot - beta*inhib - alpha)/temp
