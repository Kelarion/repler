...
for sess in os.listdir(constants.SELECTIVITY_DIR / f'{subj}/'):
    ...
    X = dset.data['neural']
    mavg = np.apply_along_axis(np.convolve, 0, dset.data['neural'].sum(1), np.ones(100)/100)
    deez = mavg[100:-100].min(0) > 0
    
    X = X[...,deez]
    X /= np.sqrt(np.mean((X-X.mean((0,1))**2)))
...
for i, this_sess in enumerate(all_neurs.values()):
    for j, (lab, rep) in enumerate(this_sess.items()):
        ...
        if lab in pp_trn.keys():
            pp_trn[lab] = np.concatenate([pp_trn[lab], X_trn], axis=2)
            pp_tst[lab] = np.concatenate([pp_tst[lab], X_tst], axis=2)
        else:
            pp_trn[lab] = X_trn
            pp_tst[lab] = X_tst
