...
# remainder
untrn = plt.plot(epochs, np.array(out_PS)[...,output_task.dim_output:].mean(1))[0]
plt.semilogx()
...
# remainder
untrn = plt.plot(epochs, np.array(inp_PS)[...,input_task.dim_output:].mean(1))[0]
plt.semilogx()
...
almost_all_CCGP = util.group_mean(np.array(all_CCGP), mask)
...
z = model(inputs[idx,...]).detach().numpy()
...
colorby = util.decimal(fake_task(cond))
