...
ax.scatter(ys[:,0],ys[:,1],ys[:,2], s=2, c=fake_labels[:,0])
...
if surf == 'torus':
    ...
    ys = np.stack([np.cos(np.pi*fake_labels[:,0]),np.sin(np.pi*fake_labels[:,0])])
    for i in range(1,num_var):
        ys = np.concatenate([ys+(r**i)*ys*np.cos(np.pi*fake_labels[:,i]), 
                             (r**i)*np.sin(np.pi*fake_labels[:,i])[None,:]])
    ys = ys.T
elif surf == 'flat_torus':
    r = 2/(1 + 5 ** 0.5)
    
    ys = np.stack([np.cos(np.pi*fake_labels[:,0]),
                   np.sin(np.pi*fake_labels[:,0]),
                   np.cos(np.pi*fake_labels[:,1]),
                   np.sin(np.pi*fake_labels[:,1])]).T
elif surf == 'moebius':
    u = np.pi*fake_labels[:,1] + np.pi
    v = fake_labels[:,0]
    ys = np.stack([(1+np.cos(u/2)*v/2)*np.cos(u),
                   (1+np.cos(u/2)*v/2)*np.sin(u),
                   np.sin(u/2)*v/2]).T

elif surf == 'flat_moebius':
    u = np.pi*fake_labels[:,0] + np.pi
    v = np.pi*fake_labels[:,1] + np.pi
    
    # ys = np.stack([np.sin(),
    #                ])

elif surf == 'blanusa_moebius':
    ys = util.blanusa_moebius(1.8*fake_labels[:,0], np.pi*fake_labels[:,1] + np.pi).T
    ...
    theta = np.stack([np.arccos(fake_labels[:,0]),
                      np.pi*fake_labels[:,1]+np.pi]).T
    ys = np.stack([np.cos(theta[:,0]),
                   np.sin(theta[:,0])*np.cos(theta[:,1]),
                   np.sin(theta[:,0])*np.sin(theta[:,1])]).T

ys -= ys.mean(0)
z = ys@basis[:,:ys.shape[-1]].T
...
if ys.shape[-1] == 3:
    ...
    emb = umap.UMAP(n_components=3).fit_transform(ys)
    # plt.scatter(emb[:,0],emb[:,1],c=fake_labels[:,1])
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
ax.scatter(emb[:,0],emb[:,1],emb[:,2], s=2, c=fake_labels[:,0])
util.set_axes_equal(ax)
...
# ax.scatter(fake_labels[:,0],fake_labels[:,1],ys[:,0],c=w, s=1)
ax.scatter(emb[:,0],emb[:,1],emb[:,2],c=cols, s=1)

util.set_axes_equal(ax)

#%%
which_neuron = np.random.choice(dim)
# which_neuron = 9
# which_neuron = 0

num_rep = 3

# activity = (z[:1000,which_neuron])*(z[:1000,which_neuron]>=0)
activity = z[:1000,which_neuron]
# activity = ys[:1000,which_neuron]

d0 = fake_labels[:,0].max() - fake_labels[:,0].min()
# d1 = fake_labels[:,1].max() - fake_labels[:,1].min()
d1 = 2

# plt.scatter(fake_labels[:,0],fake_labels[:,1],c=activity)
