...
for j,sess in enumerate(os.listdir(f"{LOAD_DIR}/{dset}/{subj}/")):
    ...
    deez = beh_df['blockType'] != 'Change'
    
    block = (beh_df['blockType'][deez] == 'Whisker').to_numpy()
    trial = (beh_df['trialType'][deez] == 'Stim_Som_NoCue').to_numpy()
    resp = np.unique(beh_df['response'][deez], return_inverse=True)[1]
    
    ## Block 0 = vis, 1 = whisk
    ## Trial 0 = vis, 1 = whisk
    ## Response 1 = right lick, 2 = left lick, 0 = no lick
    
    correct = np.where(trial == block, 2-block, 0)
    
    n_neur = len(nwb.units.id)
    
    bin_cntr = nwb.trials[phase][deez]
