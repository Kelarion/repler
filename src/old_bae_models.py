...
class BiPCA(BMF):
    ...
    def EStep(self, S, X, inplace=True):
        ...
        newS = old_bae_search.bpca(XW, S, self.scl, 
...
#             newS = old_bae_search.sbmf(XW, 1.0*self.S, self.W.T@self.W, 
#                 StS=StS, N=self.n, 
#                 alpha=self.alpha, beta=self.beta, temp=self.temp)
#         else:
#             newS = old_bae_search.sbmf(XW, 1.0*self.S, self.scl, 
#                 alpha=self.alpha, beta=self.beta, temp=self.temp)
...
class KernelBMF(BMF):
    ...
    def EStep(self, S, X, inplace=True):
        ...
        if self.kernel_input: # the input is a kernel matrix
            newS = old_bae_search.kerbmf2(X, S, scl=self.scl, 
                StS=self.StS, N=self.n, inplace=inplace,
                alpha=self.sparse_reg, beta=self.tree_reg, temp=self.temp)

        else: # the input is a feature matrix
            newS = old_bae_search.kerbmf(X, S, scl=self.scl, 
...
class KernelBMF2(BMF):
    ...
    def EStep(self, S, X, inplace=True):
        if self.kernel_input: 
            newS = old_bae_search.kerbmf2(
                X, 
                S, scl=self.scl, 
                StS=self.StS, N=self.n, inplace=inplace,
                alpha=self.sparse_reg, beta=self.tree_reg, temp=self.temp)
        else: 
            newS = old_bae_search.kerbmf3(
                X, 
                S, 
                scl=self.scl, 
                StS=self.StS, StX=self.StX, N=self.n, inplace=inplace,
...
class JBMF(BMF):
    ...
    def EStep(self, S, X, inplace=True):
        ...
        if self.tree_reg > 1e-6:
            StS = 1.0*(S.T@S)
            newS = old_bae_search.sbmf(XW, 1.0*S, WtW + J, 
                StS=StS, N=self.n, 
                beta=self.tree_reg, alpha=self.sparse_reg,
                temp=self.temp)
        else:
...
class SCPD(BMF):
    ...
    def EStep(self, S, X):
        ...
        if self.tree_reg > 1e-6:
            StS = 1.0*S.T@S
            newS = old_bae_search.sbmf(XW, 1.0*S, WtW, 
                StS=StS, N=self.n, 
                beta=self.tree_reg, alpha=self.sparse_reg,
                temp=self.temp)
        else:
...
class ConvBMF(BMF):
    ...
    def EStep(self, S, X):
        ...
        newS = old_bae_search.convbmf(XW, S_, WtW, 
...
class SpikeRRNMF(BMF):
    ...
    def EStep(self, S, X, slab=True):
        ...
        newS, newZ = old_bae_search.snmf(XW=XW, 
                                    S=S,
                                    Z=self.Z,
                                    WtW=WtW,
                                    tau=self.slab_prior,
...
class Buffet(BMF):
    ...
    def EStep(self, S, X):
        ...
        newS = old_bae_search.gauss_bern(X, 1.0*S, self.covW, self.meanW,
