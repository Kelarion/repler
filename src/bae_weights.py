"""
bae_weights.py  --  the weight side of the BMF stack
=======================================================

The *weight side* of the refactored BMF stack (see bae_models.py for the
...
import bae_search
from bae_search import make_dense_search
    ...
    def backward(self, S, X, method='grad'):
        if not self._multi:
            ...
            if method == 'grad':
                ...
                ...  # per-column NNLS, pure-nonneg case (old_bae_models.SemiBMF.MStep:360)
            elif method == 'svd':
...
class CPOperator(LinearOperator):
    """L(S) = einsum('ck,tk,nk->ctn', S, U, V) + b.  Backs SCPD (old_bae_models.SCPD).
    ...
    def forward(self, S):                          # old_bae_models.SCPD.__call__:1086
        if not self._multi:
            with torch.no_grad():
    ...
    def drive(self, X):                             # old_bae_models.SCPD.EStep:1145
        if not self._multi:
            with torch.no_grad():
    ...
    def gram(self):                                 # old_bae_models.SCPD.EStep:1146
        if not self._multi:
            with torch.no_grad():
    ...
    def decode(self, S_t):                          # old_bae_models.SCPD.MStep:1175
        if not self._multi:
    ...
    def init_params(self, X, dim_hid, hot_start=False, opt_alg=optim.Adam, **opt_args):
        ...
        # prior, so -- unlike old_bae_models.SCPD.initialize:1102 -- this sets only the
    ...
    def backward(self, S, X):                       # autograd; PR on V + L1 on U,V (:1180)
        ...
        # each /dim_hid exactly as old_bae_models.SCPD.MStep:1187-1190.  Returns the
...
class ReducedRankOp(LinearOperator):
    ...
    def forward(self, S):                           # old_bae_models.RRBMF.__call__:1347
        if not self._multi:
            with torch.no_grad():
    ...
    def drive(self, X):                             # old_bae_models.RRBMF.EStep:1407
        if not self._multi:
            with torch.no_grad():
                Xt = torch.as_tensor(np.asarray(X), dtype=self.b.dtype)
    ...
    def gram(self):                                 # old_bae_models.RRBMF.EStep:1408
        if not self._multi:
            with torch.no_grad():
                beta = self._beta()
    ...
    def decode(self, S_t):                          # old_bae_models.RRBMF.MStep:1437
        if not self._multi:
    ...
    def _reg_loss(self, loss, beta):
        ...
        # normalized exactly as old_bae_models.RRBMF.MStep:1449-1454.
...
class ConvOperator(LinearOperator):
    """L(S) = conv1d(S, K) + b.  Backs ConvBMF (old_bae_models.ConvBMF).
    ...
    def forward(self, S):                           # old_bae_models.ConvBMF.__call__:1222
        with torch.no_grad():
            Xhat = F.conv1d(S, self.K.flip([2]), padding=self.pad) + self.b
        return Xhat.detach().numpy()

    def drive(self, X):                             # old_bae_models.ConvBMF.EStep:1266
        with torch.no_grad():
            Kt = self.K.transpose(0, 1)
            XW = F.conv1d(X - self.b, Kt, padding=0)
        return XW.detach().numpy()                  # (n, dim_hid, t)

    def gram(self):                                 # old_bae_models.ConvBMF.EStep:1267
        with torch.no_grad():
            Kt = self.K.transpose(0, 1)
    ...
    def search(self, XW, S, Z, WtW, StS, N, temp, alpha, beta, tau, sigma2,
        ...
        return old_bae_search.convbmf(
            XW, S, WtW, temp=temp,
            beta=self.feature_sparsity,
            alpha=self.time_sparsity,
            l1_reg=self.sparse_reg)

    def decode(self, S_t):                          # old_bae_models.ConvBMF.MStep:1299
